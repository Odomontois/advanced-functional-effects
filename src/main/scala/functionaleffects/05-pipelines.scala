  ...
  def spec =
    suite("Introduction") {
      ...
      test(">>>") {
        ...
        /** EXERCISE
          *
          * Using `.transduce`, transform the elements of this stream by the
          * provided pipeline.
          */
        test("transduce") {
      ...
      test("splitLines") {
        ...
        test("fromFunction") {
          def parseInt: ZPipeline[Any, Nothing, String, Int] =
            ZPipeline.mapZIO(s => ZIO.attempt(s.toInt).orDie)
...
object Graduation extends ZIOSpecDefault {
  def rechunkChannel[A, S](init: S)(
      f: (Chunk[A], S) => (Vector[Chunk[A]], S)
  ): ZChannel[Any, ZNothing, Chunk[A], Any, ZNothing, Chunk[A], S] =
    ZChannel.readWith(
      { chunk =>
        val (emit, next) = f(chunk, init)
        ZChannel.writeAll(emit: _*) *> rechunkChannel(next)(f)
      },
      err => err,
      done => ZChannel.succeed(init)
    )

  def rechunkWith[A, S](init: S)(
      makeChunks: (Chunk[A], S) => (Vector[Chunk[A]], S)
  )(mapResult: S => Chunk[A]): ZPipeline[Any, Nothing, A, A] =
    ZPipeline.fromChannel(
      rechunkChannel(init)(makeChunks).flatMap { s =>
        val fin = mapResult(s)
        if (fin.isEmpty) ZChannel.unit else ZChannel.write(fin)
      }
    )

  def rechunk[A](n: Int): ZPipeline[Any, Nothing, A, A] =
    rechunkWith(Chunk.empty[A]) { (next: Chunk[A], leftover) =>
      val full = leftover ++ next
      val loSize = full.length % n
      val (emits, lo) = full.splitAt(full.length - loSize)

      (emits.grouped(n).toVector, lo)
    }(x => x)
